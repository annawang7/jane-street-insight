...
let can_move t ~row ~col =
  ...
  if col>=0 && row = t.height  && col < t.width-1 then true
  else if (col<0 || col>=t.width-1 || row<0 || not (Board.is_empty t.board {col=col ; row=row})) then false
  else true
;;
...
let drop t =
  ...
  ignore (Board.add_piece t.board ~moving_piece:t.moving_piece ~col:t.moving_piece_col);
  new_moving_piece t
;;
...
let tick t =
  ...
  if (t.moving_piece_row > 0 && Board.is_empty t.board { col=t.moving_piece_col ; row=t.moving_piece_row-1})
  then t.moving_piece_row <- t.moving_piece_row-1
  else (
    ignore (Board.add_piece t.board ~moving_piece:t.moving_piece ~col:t.moving_piece_col);
    ignore (new_moving_piece t)
  )
;;
