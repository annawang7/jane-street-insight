...
def add(symbol, direction, price, size):
    ...
    #if symbol == "VALE":
    print("Order Placed: " + str(orders_placed) + " Size: " + str(size) + " Dir: " + direction + " Symbol: " + symbol + " Price: " + str(price) + "")
...
def buy_sell_vale():
    if vale_buy > 0 and vale_sell > 0:
        if (pending_buy_orders["VALE"] < 10 + pending_buy_orders["VALE"] + positions["VALE"] < 10):
        ...
        elif (pending_sell_orders["VALE"] < 10 and positions["VALE"] - pending_sell_orders["VALE"] > -10):
            global vale_sell_pending_id
            if vale_sell_pending_id:
                print("cancel sell order")
                cancel(vale_sell_pending_id)
                vale_sell_pending_id = None
            add("VALE", "SELL", vale_sell - 1, 10 + positions["VALE"])
            vale_sell_pending_id = orders_placed
        print("Vale Pending Sell: " + str(pending_sell_orders) + " Pending Buy: " + str(pending_buy_orders))

def listen_for_errors(server_msg):
    if (server_msg["type"] == "reject"):
        print("Order failed, id: " + str(id) + " " + server_msg["error"])
